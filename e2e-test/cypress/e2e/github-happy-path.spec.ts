...
describe("Test with Guest Sign-in", () => {
  ...
  const component = 'https://github.com/janus-idp/backstage-showcase/blob/main/catalog-entities/all.yaml'
  ...
  it("Verify Profile is Github Account Name in the Settings page", () => {
    ...
    UIhelper.verifyHeading(`User Entity: user:default/${Cypress.env('GH_USER_ID')}`)
  })
  ...
  it('Verify that the following components were ingested into the Catalog', () => {
    ...
    UIhelper.verifyRowsInTable(['ArgoCD', 'GitHub Showcase repository', 'KeyCloak', 'PostgreSQL cluster', 'S3 Object bucket storage']);

    UIhelper.selectMuiBox('Kind', "System");
    UIhelper.verifyRowsInTable(['Janus-IDP']);

    UIhelper.selectMuiBox('Kind', "User");
    UIhelper.verifyRowsInTable(['subhashkhileri', 'josephca', 'gustavolira', 'rhdh-qe'])
  })
});
